...
def run_test(test):
    m = re.match(r'(\w+)\s*(.*)', test['line'])
    if m.group(1) == 'echo':
        test['type'] = 'info'
        print(colors.INFO + m.group(2) + colors.END)
    elif os.path.isdir(m.group(1)):
        test['type'] = 'dir'
        olddir = os.getcwd()
        os.chdir(m.group(1))
        chdirargs = "make -s testing".split()
        p = subprocess.Popen(chdirargs)
        wait_with_signal(p, test)
        os.chdir(olddir)
    else:
        test['type'] = 'test'
        test_dir = test['dir']
        ...
        if test_dir == '.':
            ...
            sys.stderr.write(colors.FAILURE + ">>>> ERROR: " + colors.END)
            sys.stderr.write(test_dir + ' - ' + "nested testlists not supported (due to recursive make)\n")
            sys.exit()
